...
| Browser から Runtime を試す（web-demo） | `pnpm nx serve web-demo` で接続状態と GPIO Output / GPIO Input / I2C Scan ナビを確認する。[browser-polyfill.md](./browser-polyfill.md)。GPIO Output は Start / Stop で LED Blink。配線は [gpio-led-blink.md](../examples/gpio-led-blink.md) |
