...
GPIO Input の配線は [回路仕様](../examples/gpio-input.md)（BCM 5 / 物理 pin 29 / タクトスイッチ + 10kΩ プルアップ。旧 [gc/gpio/button](https://github.com/chirimen-oh/chirimen/tree/master/gc/gpio/button) と同じピン）。Runtime が `Connected` のとき Start で GPIO5 を input で開き、`onchange` で現在値 `0` / `1` を realtime 表示する。Read で再読込、Stop / 画面離脱 / reload / WebSocket 切断 / Runtime 停止で unsubscribe と unexport をする。再接続後は自動再開せず、同じ GPIO5 を再度 Start できる。操作手順の本ガイドは [gpio-input.md](./gpio-input.md)。
