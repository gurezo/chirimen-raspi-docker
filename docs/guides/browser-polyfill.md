...
workspace/my-first-example/polyfill.js
...
`pnpm build`（CI 含む）でも同じ bundle が走る。`dist` の成果物は git 管理外。my-first-example / GPIO LED Blink / GPIO Input / I2C Scan / PIR / ADT7410 / SHT30 / ADS1115 サンプル用の `polyfill.js` は bundle 時にコピーし、サンプルから `<script src="./polyfill.js">` で読む。
