...
| `scripts/build-server.mjs`（削除済み） | esbuild で `apps/runtime` を bundle。呼び出し元だった `Dockerfile.32bit` は削除済み（`package.json` 未登録） | delete（完了） | #341 | 削除済み。32-bit workaround 専用で他用途なし。esbuild 導入理由は [compatibility-32bit.md](./compatibility-32bit.md) に Historical として保存 |
