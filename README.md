...
Raspberry Pi の対応状態は、モデル名だけではなく Hardware Capability Detection と Runtime Backend の実機検証結果として記録します。未検証項目は `Supported` と書きません。正本は [docs/architecture/docker.md](docs/architecture/docker.md#compatibility-matrix) です。
...
- **Pi 3 32-bit**（`armv7l` / `6.18.34+rpt-rpi-v7`）: host path 確認のみ。Runtime E2E は未検証
...
- **Pi 3 B+（#97）**: 64-bit OS（`aarch64`）で `gpio=sysfs` / `i2c=i2c-dev` を実機確認済み。詳細は [docker.md](docs/architecture/docker.md) の「Pi 3 B+ 実機検証」。A+ は未検証
