...
Browser からはこの Runtime API を呼びません。Scan は Public polyfill / protocol に置かず、web-demo が `navigator.requestI2CAccess()` と既存の `open` / `writeByte` で走査を合成します（Demo-only。[#114](https://github.com/gurezo/chirimen-raspi-docker/issues/114) / UI [#115](https://github.com/gurezo/chirimen-raspi-docker/issues/115) / 実機検証 [#116](https://github.com/gurezo/chirimen-raspi-docker/issues/116)。親 [#52](https://github.com/gurezo/chirimen-raspi-docker/issues/52)）。経路の正本は [protocol.md の I2C Scan API flow](docs/architecture/protocol.md#i2c-scan-api-flow114)。検証仕様は [docs/examples/i2c-scan.md](docs/examples/i2c-scan.md)（ADT7410 / `0x48`）。
