...
- **Pi 3 B+（#97）**: Raspbian OS 64-bit（`aarch64`）で `gpio=sysfs` / `i2c=i2c-dev` を実機確認済み。詳細は [docker.md](docs/architecture/docker.md) の「Pi 3 B+ 実機検証」。A+ は未検証
- **Pi 4（#98）**: Raspbian OS 64-bit（`aarch64`）で `gpio=sysfs` / `i2c=i2c-dev` を実機確認済み。詳細は [docker.md](docs/architecture/docker.md) の「Pi 4 実機検証」
